...
## Modify missing values in dataset from ? to NA
  ## there is no missing data for these dates so no way to test the following statement
  ##datafl$Sub_metering_2[datafl$Sub_metering_2 == "?"] <- NA
  
## Plot1
  pdf(file="plot1.pdf")
  hist(datafl$Global_active_power, col='red', xlab="Global Active Power (kilowatts)",
